...
## Shared Between Threads
### Heap
heap은 런타임에 클래스 인스턴스와 배열을 할당하는 데 사용됩니다. 프레임이 생성된 후 크기가 변경되도록 설계되지 않았기 때문에 배열과 객체는 스택에 저장할 수 없습니다. 프레임은 heap의 개체 또는 배열을 가리키는 참조만 저장합니다. local variables array(in each frame)의 기본 변수 및 참조와 달리 객체는 항상 heap에 저장되므로 메서드가 종료될 때 제거되지 않습니다. 대신 개체는 가비지 수집기에 의해서만 제거됩니다.
...
| Value             | Desc.|
|-------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `aload_0`         | 이 opcode는 aload_<n> 형식의 opcode 그룹 중 하나입니다. 모두 피연산자 스택에 개체 참조를 로드합니다. <n>은 액세스 중인 로컬 변수 배열의 위치를 참조하지만 0, 1, 2 또는 3일 수 있습니다. 객체 참조 iload_<n>, lload_<가 아닌 값을 로드하기 위한 다른 유사한 opcode가 있습니다. n>, float_<n> 및 dload_<n> 여기서 i는 `int`, l은 `long`, f는 `float`, d는 `double`입니다. 인덱스가 3보다 높은 지역 변수는 `iload`, `lload`, `float`, `dload` 및` aload`를 사용하여 로드할 수 있습니다. 이러한 opcode는 모두 로드할 로컬 변수의 인덱스를 지정하는 단일 피연산자를 사용합니다. |
